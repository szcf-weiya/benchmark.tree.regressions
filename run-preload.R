source("R/datasets.R")
source("R/evaluate.R")
source("R/methods.R")

lst_methods = c(rep("bart_fit", 3),
                rep("xbart_fit", 3),
                rep("mars_fit", 4),
                rep("ranger_fit", 3),
                rep("rf_fit", 3),
                rep("xgboost_fit", 1))
names_lst_methods = c("BART_200", "BART_500", "BART_1000",
                       "XBART_10_10", "XBART_5_10", "XBART_10_5",
                       "MARS_d1", "MARS_d2", "MARS_d1_df", "MARS_d2_df",
                       "ranger_200", "ranger_500", "ranger_1000",
                       "randomForest_200", "randomForest_500", "randomForest_1000",
                       "XGBoost")
stopifnot(length(lst_methods) == length(names_lst_methods))
names(lst_methods) = names_lst_methods
lst_methods_paras = list(list(ntree = 200), list(ntree = 500), list(ntree = 1000),
                         list(num_trees = 10, num_sweeps = 10),
                         list(num_trees = 5, num_sweeps = 10),
                         list(num_trees = 10, num_sweeps = 5),
                         list(degree = 1, df.correct = FALSE),
                         list(degree = 2, df.correct = FALSE),
                         list(degree = 1, df.correct = TRUE),
                         list(degree = 2, df.correct = TRUE),
                         list(num.trees = 200), list(num.trees = 500), list(num.trees = 1000),
                         list(ntree = 200), list(ntree = 500), list(ntree = 1000),
                         NULL)
stopifnot(length(lst_methods) == length(lst_methods_paras))
